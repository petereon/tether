...
| Wifi | `"wifi:<ip>"` | Not supported directly — board must already run a bootstrapped runtime, now reachable via `tether provision-wifi` | **Works today**, verified end-to-end against real ESP32 hardware (added 2026-07-25, see `docs/superpowers/specs/2026-07-25-wifi-upload-design.md`; hardware-verified 2026-07-25 via `tether[cli]`). `tether provision-wifi` uploads a `boot.py` (`src/tether/provisioning.py`'s `generate_wifi_boot()`) that runs automatically on every MicroPython boot: it reads credentials from `/tether_wifi.json` (absent = no-op, falls through to idle REPL exactly as before this existed), joins wifi with a bounded ~15s timeout (never permanently locking out serial on a bad password), opens a TCP listener on `DEFAULT_PORT` (8765), accepts exactly **one** connection for that boot cycle, and - if `/tether_app.py` is present - bridges it into the same generated dispatch loop serial uses, via a `_tether_stream_override` global that `generate_bootstrap()` checks before falling back to stdio (see step 5's "Current status" note above). `tether status`/`tether unprovision-wifi` round out the CLI. A real `mcu.connect("wifi:<ip>")` session was verified against a real ESP32 too: a PC-to-MCU call (`add(3, 4)`), an MCU-to-PC reverse call from inside that same handler (`@pc.export` invoked from MCU code, itself dependent on the reverse call's real return value flowing back over the wire), and remote-exception propagation (a MicroPython-only `NotImplementedError` from an unsupported slice operation correctly surfaced PC-side as `RemoteError`) all confirmed working over the real socket bridge - not just the real-MicroPython-interpreter test suite. Real, accepted limitations: `/tether_app.py` (the program a wifi connection actually bridges into) is only ever written by a prior *serial* `mcu.connect(...)` session - wifi never pushes code, and there's no hash-check on the wifi path, so a wifi connection silently runs whatever was uploaded last over serial (confirmed directly during hardware verification: an edited script kept running its old version over wifi until a fresh serial upload); the TCP listener is **unauthenticated** - anyone on the same network can be the one connection a boot cycle accepts; no re-listen once the one accepted connection drops (needs a physical reset or a fresh `provision-wifi` run - see "Explicitly out of scope" in the design spec for why). **`tether status` leaves the board unable to accept a new wifi connection afterward** (found during hardware verification, not anticipated in the original design): `status` hard-resets the board and interrupts `boot.py` via raw-REPL entry to query it, then exits raw REPL back to the plain interactive REPL - which does NOT re-trigger `boot.py` the way an actual hardware reset does, so the listener `boot.py` had opened is gone (its socket object was a local variable in the now-interrupted script, unreferenced and effectively dead) even though `status` just reported "connected". The already-established wifi *association* survives the interrupt (confirmed - `WLAN.isconnected()` stays true, since that's a firmware/driver-level state independent of the interrupted Python script), but the TCP *listener* does not. Practical effect: `provision-wifi` → `status` → `mcu.connect("wifi:<ip>")` will time out on the connect, even though `status` just said "connected" - `status` must not be checked between provisioning and connecting. A fix (resetting the board again after the status query, mirroring `provision_wifi_command`'s own double-reset pattern, so `boot.py` runs fresh and re-listens) is a reasonable follow-up, not yet implemented. Credentials stored in plaintext on-device (no secure storage exists on this hardware class). |
