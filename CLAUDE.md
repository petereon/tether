...
- **BLE only for Bluetooth, no classic BT.** BLE still never pushes
  code — only serial does (see DESIGN.md § Transports for why:
  chicken-and-egg on an unbootstrapped board). **Wifi is the exception, as
  of 2026-07-26:** `mcu.connect("wifi:<ip>")` now pushes code too, via its
  own authenticated status/upload/run protocol (see DESIGN.md § Transports'
  Wifi row and § Architecture overview step 5's correction) — a
  provisioned board's `boot.py` no longer needs a prior serial upload to
